...
```bash
pnpm add @ssortia/shadcn-zod-bridge
```

```bash
npm install @ssortia/shadcn-zod-bridge
```

Подключите стили в вашем приложении:

```ts
import "@ssortia/shadcn-zod-bridge/styles";
```
...
```tsx
import {z} from "zod";
import {ZodForm, TextField, NumberField, SelectField, Button} from "@ssortia/shadcn-zod-bridge";
...
```tsx
import {z} from "zod";
import {AutoForm} from "@ssortia/shadcn-zod-bridge";
...
- [ ] Все тесты проходят: `pnpm --filter @ssortia/shadcn-zod-bridge test:run`
- [ ] Сборка без ошибок: `pnpm --filter @ssortia/shadcn-zod-bridge build`
- [ ] Обновлена версия в `packages/ui/package.json` по [semver](https://semver.org/lang/ru/)
- [ ] Обновлён `CHANGELOG.md` (если есть)
- [ ] Коммит и тег созданы: `git tag v0.1.0 && git push --tags`
- [ ] Выполнен вход в npm: `npm whoami` (если нет — `npm login`)

### Команды публикации

```bash
# 1. Обновить версию (patch / minor / major)
npm version patch --workspace=packages/ui

# 2. Собрать пакет
pnpm --filter @ssortia/shadcn-zod-bridge build

# 3. Проверить, что попадёт в пакет
...
> Для публикации pre-release версии:
> ```bash
> npm version prerelease --preid=beta --workspace=packages/ui
> npm publish --workspace=packages/ui --access public --tag beta
> ```
